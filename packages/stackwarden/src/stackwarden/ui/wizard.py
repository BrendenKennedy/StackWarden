...
from stackwarden.domain.variants import normalize_variants, validate_variant_flags
...
def build_command(selection: WizardSelection) -> str:
    """Generate a deterministic ``stackwarden ensure`` CLI string."""
    parts = [
        "stackwarden", "ensure",
    ...
    *,
    ...
    if not profile_id:
        if defaults and default_profile:
            ...
            if not profiles:
                con.print("[red]No profiles found. Run 'stackwarden init' first.[/red]")
                raise SystemExit(1)
