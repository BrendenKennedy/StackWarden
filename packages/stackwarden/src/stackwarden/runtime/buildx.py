...
def build(
    *,
    ...
    labels:
        ``--label`` key=value pairs (StackWarden metadata injected here).
    load:
...
def _retry_cmd_with_lower_parallelism(cmd: list[str]) -> list[str]:
    """Return a modified command with reduced STACKWARDEN_BUILD_JOBS when present."""
    updated = list(cmd)
    for i, token in enumerate(updated):
        if token != "--build-arg" or i + 1 >= len(updated):
            continue
        arg = updated[i + 1]
        if not arg.startswith("STACKWARDEN_BUILD_JOBS="):
            continue
        value = arg.split("=", 1)[1]
        try:
            jobs = int(value)
        ...
        if lowered == jobs:
            return updated
        updated[i + 1] = f"STACKWARDEN_BUILD_JOBS={lowered}"
        return updated
    return updated
