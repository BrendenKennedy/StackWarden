...
from stacksmith.config import get_stack_origin, get_stacks_dir, list_stack_ids, load_stack, load_stack_spec_raw
from stacksmith.domain.errors import StackNotFoundError
...
async def list_stacks(response: Response, schema: str | None = Query(default=None)):
    ...
    if skipped:
        response.headers["X-Stacksmith-Stacks-Skipped"] = str(skipped)
