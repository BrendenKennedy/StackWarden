...
def canonicalize(
    ...
    *builder_version_override* lets callers (e.g. ``verify``) replay a
    fingerprint that was computed with an older Stacksmith version.
    """
    pip_entries = sorted(
...
def compute_template_hash(template_path: Path) -> str:
    """SHA-256 of the template content, excluding the ``stacksmith_template_hash=``
    line to avoid a circular dependency."""
    lines = template_path.read_text().splitlines(keepends=True)
    filtered = [line for line in lines if not line.strip().startswith("# stacksmith_template_hash=")]
    return hashlib.sha256("".join(filtered).encode("utf-8")).hexdigest()


def extract_template_version(template_path: Path) -> int:
    """Parse ``# stacksmith_template_version=N`` from the first lines of a template."""
    for line in template_path.read_text().splitlines()[:5]:
        m = re.match(r"^#\s*stacksmith_template_version\s*=\s*(\d+)", line)
        if m:
...
def generate_tag(
    ...
    Format: ``local/stacksmith:{stack}-{profile}-{cuda}-{serve}-{api}-{h12}``
    """
    h12 = fp[:12]
    ...
    return "local/stacksmith:" + "-".join(parts)
