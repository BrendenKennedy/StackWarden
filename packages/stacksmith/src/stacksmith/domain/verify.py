...
from stacksmith.domain.hashing import fingerprint as compute_fingerprint
from stacksmith.domain.models import ArtifactRecord, Profile, StackSpec
...
def verify_artifact(
    ...
    *,
    ...
    report.label_fingerprint = labels.get("stacksmith.fingerprint")
    ...
    if profile_path.exists() and stack_path.exists():
        try:
            ...
            base_digest = labels.get("stacksmith.base_digest") or record.base_digest
            template_hash = labels.get("stacksmith.template_hash") or record.template_hash
            builder_version = labels.get("stacksmith.builder_version", "")

            variants: dict[str, str] | None = None
            variant_label = labels.get("stacksmith.variants")
            if variant_label:
