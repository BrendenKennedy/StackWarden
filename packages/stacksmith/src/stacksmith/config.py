...
from stacksmith.domain.composition import compose_stack
...
from stacksmith.domain.models import BlockSpec, Profile, StackRecipeSpec, StackSpec
from stacksmith.paths import get_config_path
...
def _data_dir() -> Path:
    override = os.environ.get("STACKSMITH_DATA_DIR")
    if override:
...
def _configured_local_overrides_dir() -> Path | None:
    if os.environ.get("STACKSMITH_DATA_DIR"):
        return None
    data = _load_config_data()
    remote = data.get("remote_catalog", {}) or {}
    enabled = bool(remote.get("enabled"))
    if not enabled:
        return None
    local_overrides = str(
        remote.get("local_overrides_path", "~/.local/share/stacksmith/local-catalog")
    ).strip()
...
def _spec_roots(kind: str) -> list[Path]:
    ...
    if not os.environ.get("STACKSMITH_DATA_DIR"):
...
def _safe_resolve(base: Path, name: str, suffix: str) -> Path:
    ...
    if not path.is_relative_to(base.resolve()):
        raise StacksmithError(f"Invalid identifier (path traversal rejected): {name!r}")
    return path
...
def load_profile(profile_id: str) -> Profile:
    ...
    try:
        ...
        raise StacksmithError(f"Invalid profile '{profile_id}': {exc}") from exc
...
def load_stack(stack_id: str) -> StackSpec:
    ...
    try:
        ...
        raise StacksmithError(f"Invalid stack '{stack_id}': {exc}") from exc
...
def load_block(block_id: str) -> BlockSpec:
    ...
    try:
        ...
        raise StacksmithError(f"Invalid block '{block_id}': {exc}") from exc
...
def compatibility_strict_default() -> bool:
    return os.environ.get("STACKSMITH_COMPAT_STRICT", "").strip().lower() in {"1", "true", "yes", "on"}
...
def tuple_layer_mode() -> str:
    ...
    if cfg.tuple_layer_mode and cfg.tuple_layer_mode.strip():
        ...
        if mode in {"off", "shadow", "warn", "enforce"}:
            return mode
    mode = os.environ.get("STACKSMITH_TUPLE_LAYER_MODE", "enforce").strip().lower()
    if mode in {"off", "shadow", "warn", "enforce"}:
        return mode
    return "enforce"
...
# ---------------------------------------------------------------------------
# App-level config (~/.config/stacksmith/config.yaml)
# ---------------------------------------------------------------------------
...
class AppConfig:
    ...
    def __init__(self, data: dict | None = None) -> None:
        ...
        from stacksmith.domain.registry_policy import RegistryPolicy
        ...
        self.remote_catalog_local_path: str = (
            str(remote.get("local_path", "~/.local/share/stacksmith/remote-catalog")).strip()
        ...
        self.remote_catalog_local_overrides_path: str = (
            str(
                remote.get(
                    ...
                    "~/.local/share/stacksmith/local-catalog",
                )
            ).strip()
