...
from stacksmith.config import templates_roots
...
def _jinja_env() -> Environment:
    ...
    if not roots:
        from stacksmith.config import _DEFAULT_DATA_DIR
...
def render_requirements(
    ...
    if not stack.components.pip:
        return False
    from stacksmith.domain.pip_compatibility import apply_overrides
...
def render_dockerfile(
    ...
    from stacksmith.domain.pip_compatibility import get_pip_install_options
...
def build_overlay(
    ...
    with tempfile.TemporaryDirectory(prefix="stacksmith_") as tmpdir:
        ctx = Path(tmpdir)
