...
from stacksmith.domain.drift import detect_drift, drift_summary, is_stale
from stacksmith.domain.enums import ArtifactStatus, BuildStrategy
from stacksmith.domain.errors import BuildError, CancellationRequestedError, DriftError
from stacksmith.domain.locking import acquire_lock, compute_variant_hash
...
class PullBuilder:
    ...
    def execute(
        ...
        from stacksmith.builders.pull import build_pull
...
def execute_plan(
    ...
    *,
    ...
    """
    try:
        variant_data = _json.loads(plan.artifact.labels.get("stacksmith.variants", "{}"))
    except _json.JSONDecodeError:
        log.warning("Malformed stacksmith.variants label, treating as empty")
        variant_data = {}
...
def _execute_plan_locked(
    ...
    *,
    ...
    fh = None
    if build_log_path:
        from stacksmith.logging import add_file_handler
...
def _capture_manifest(
    ...
    try:
        from stacksmith.runtime.manifest_capture import capture_manifest
        from stacksmith.domain.manifest import save_manifest
...
def _capture_snapshots(
    ...
    try:
        from stacksmith.domain.snapshots import artifact_dir, write_snapshot_files
...
def _run_hooks(
    ...
    if importlib.util.find_spec("stacksmith.hooks") is None:
        return
...
def _make_record(plan: Plan, status: ArtifactStatus) -> ArtifactRecord:
    ...
    stack_schema_version = int(plan.artifact.labels.get("stacksmith.schema_version", "1") or "1")
    profile_schema_version = int(plan.artifact.labels.get("stacksmith.profile_schema_version", "1") or "1")
    block_schema_version = int(plan.artifact.labels.get("stacksmith.block_schema_version", "1") or "1")
