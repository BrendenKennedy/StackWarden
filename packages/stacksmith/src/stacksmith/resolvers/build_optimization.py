...
from stacksmith.domain.models import BuildOptimizationDecision, Profile, StackSpec
...
def compute_build_optimization(profile: Profile, stack: StackSpec) -> BuildOptimizationDecision:
    ...
    build_args = {
        "STACKSMITH_BUILD_JOBS": str(parallel_jobs),
        "STACKSMITH_BUILD_MEMORY_BUDGET_GB": f"{memory_budget:.2f}",
        "STACKSMITH_EST_BUILD_MEMORY_GB": f"{est_mem:.2f}",
        "STACKSMITH_OOM_RISK": oom_risk,
    }
