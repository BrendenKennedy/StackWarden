"""Stacksmith CLI — Typer application."""
...
from stacksmith.cli_commands.high_risk import command_map as high_risk_command_map
from stacksmith.cli_commands.low_risk import command_map as low_risk_command_map
from stacksmith.cli_shared.catalog import get_catalog
from stacksmith.cli_shared.context import setup_cli
from stacksmith.cli_shared.errors import deprecated_alias_notice, exit_code_for
from stacksmith.cli_shared.io_yaml import atomic_write_spec, edit_yaml_via_editor, load_yaml_file
from stacksmith.cli_shared.render import print_rationale
...
console = Console()
log = logging.getLogger("stacksmith")
...
def _artifact_runtime_and_ports(record) -> tuple[str | None, list[int]]:
    from stacksmith.config import load_profile
    from stacksmith.domain.snapshots import artifact_dir, load_snapshot
...
def list_profiles(
    ...
    _setup(verbose)
    from stacksmith.config import get_profile_origins, list_profile_ids, load_profile
...
def list_stacks(
    ...
    _setup(verbose)
    from stacksmith.config import get_stack_origins, list_stack_ids, load_stack
...
def list_blocks(
    ...
    _setup(verbose)
    from stacksmith.config import get_block_origins, list_block_ids, load_block
...
def profiles_list_cmd(
    ...
    _deprecated_alias_notice("stacksmith profiles list", "stacksmith list profiles")
...
def profiles_create_cmd(
    ...
    _setup(verbose)
    from stacksmith.application.create_flows import create_profile
    from stacksmith.web.schemas import ProfileCreateRequest
...
def profiles_edit_cmd(
    ...
    _setup(verbose)
    from stacksmith.application.create_flows import update_profile
    from stacksmith.config import load_profile
    from stacksmith.web.schemas import ProfileCreateRequest
    from stacksmith.web.util.write_yaml import serialize_for_yaml
...
def profiles_detect_cmd(
    ...
    _setup(verbose)
    from stacksmith.web.services.host_detection import detect_server_hints
...
def profiles_wizard_cmd(
    ...
    _setup(verbose)
    from stacksmith.ui.wizard_entities import run_profile_create_wizard
...
def stacks_list_cmd(
    ...
    _deprecated_alias_notice("stacksmith stacks list", "stacksmith list stacks")
...
def stacks_show_cmd(
    ...
    """Show stack details."""
    _setup(verbose)
    from stacksmith.config import load_stack
    from stacksmith.web.util.write_yaml import serialize_for_yaml
...
def stacks_create_cmd(
    ...
    _setup(verbose)
    from stacksmith.application.create_flows import create_stack
    from stacksmith.web.schemas import StackCreateRequest
...
def stacks_edit_cmd(
    ...
    _setup(verbose)
    from stacksmith.application.create_flows import update_stack
    from stacksmith.config import load_stack
    from stacksmith.web.schemas import StackCreateRequest
    from stacksmith.web.util.write_yaml import serialize_for_yaml
...
def stacks_wizard_cmd(
    ...
    _setup(verbose)
    from stacksmith.ui.wizard_entities import run_stack_create_wizard
...
def stacks_delete_cmd(
    ...
    _setup(verbose)
    from stacksmith.config import get_stacks_dir
...
def blocks_list_cmd(
    ...
    _deprecated_alias_notice("stacksmith blocks list", "stacksmith list blocks")
...
def blocks_show_cmd(
    ...
    """Show block details."""
    _setup(verbose)
    from stacksmith.config import load_block
    from stacksmith.web.util.write_yaml import serialize_for_yaml
...
def blocks_create_cmd(
    ...
    _setup(verbose)
    from stacksmith.application.create_flows import create_block
...
def blocks_edit_cmd(
    ...
    _setup(verbose)
    from stacksmith.application.create_flows import update_block
    from stacksmith.config import load_block
    from stacksmith.web.schemas import BlockCreateRequest
    from stacksmith.web.util.write_yaml import serialize_for_yaml
...
def blocks_wizard_cmd(
    ...
    _setup(verbose)
    from stacksmith.ui.wizard_entities import run_block_create_wizard
...
def plan(
    ...
    console.print(Panel(f"[bold]Plan:[/bold] {result.plan_id}", title="Stacksmith Plan"))
...
def check_cmd(
    ...
    strict: Optional[bool] = typer.Option(
        None,
        "--strict/--no-strict",
        help="Override strict compatibility mode (defaults to STACKSMITH_COMPAT_STRICT).",
    ),
) -> None:
...
def ensure(
    ...
    from stacksmith.domain.variants import parse_variants
...
def verify(
    ...
    _setup(verbose)
    from stacksmith.domain.verify import verify_artifact, apply_fix
...
def inspect_cmd(
    ...
    """Inspect a container image and show stacksmith metadata."""
    _setup(verbose)
    from stacksmith.runtime.docker_client import DockerClient
    from stacksmith.runtime.inspect import format_image_info
    from stacksmith.catalog.store import CatalogStore
    ...
    if info.get("stacksmith"):
        console.print("\n[bold]Stacksmith Labels:[/bold]")
        for k, v in info["stacksmith"].items():
            console.print(f"  {k}: {v}")
    if cat_dict:
        console.print("\n[bold]Catalog:[/bold]")
...
def manifest_cmd(
    ...
    _setup(verbose)
    from stacksmith.catalog.store import CatalogStore
    from stacksmith.domain.manifest import load_manifest
...
def catalog_build(
    ...
    _deprecated_alias_notice("stacksmith catalog build", "stacksmith ensure")
    ensure(
        profile=profile,
        stack=stack,
        rebuild=rebuild,
...
def sbom_cmd(
    ...
    _setup(verbose)
    from stacksmith.catalog.store import CatalogStore
    from stacksmith.runtime.sbom import export_sbom
...
def status(
    ...
    console.print(Panel("[bold]Stacksmith Status[/bold]"))
...
def prune_cmd(
    ...
    try:
        ...
        from stacksmith.runtime.docker_client import DockerClient as DC
        docker = DC()
    except Exception as exc:
...
def doctor(
    ...
    console.print(Panel("[bold]Stacksmith Doctor[/bold]", subtitle=f"v{__version__}"))
    ...
    # 2. Buildx
    from stacksmith.runtime.buildx import is_available
    ...
    from stacksmith.config import list_profile_ids, load_profile
    from stacksmith.domain.enums import ContainerRuntime
...
def migrate_v1_to_v2(
    ...
    _setup(verbose)
    from stacksmith.config import get_blocks_dir, get_profiles_dir, get_stacks_dir
    from stacksmith.domain.hardware_catalog import load_hardware_catalog
...
def init(
    ...
    """Initialize Stacksmith directories and default configuration."""
    _setup(verbose)
    from stacksmith.paths import (
        get_artifacts_root,
        get_config_root,
        get_data_root,
        get_locks_root,
    ...
    if config_path.exists():
        ...
        _DEFAULT_CONFIG = (
            ...
            '  local_overrides_path: "~/.local/share/stacksmith/local-catalog"\n'
            "  auto_pull: true\n"
        )
    ...
    cfg = AppConfig.load()
    console.print(f"\n[green]Stacksmith initialized.[/green]")
    if cfg.default_profile:
        console.print(f"  Default profile: {cfg.default_profile}")
    console.print(f"\nRun [cyan]stacksmith doctor[/cyan] to verify your environment.")
...
def wizard(
    ...
    if should_run:
        ...
        if verify_after:
            try:
                from stacksmith.domain.verify import verify_artifact
                from stacksmith.runtime.docker_client import DockerClient
...
@app.command()
def version() -> None:
    """Show Stacksmith version."""
    console.print(f"stacksmith {__version__}")
